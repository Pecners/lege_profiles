---
title: "State Legislator School Profiles"
...
if (params$honorific == "Representative") {
  dist <- leges_sf |> 
    filter(house == "Assembly" & district == params$district)
} else {
  dist <- leges_sf |> 
    filter(house == "Senate" & district == params$district)
}


leaflet(dist) |> 
  addMapboxTiles(style_id = "light-v11",
                 username = "mapbox", scaling_factor = "0.5x") |> 
  addPolygons(weight = 2) |> 
  addPolygons(data = st_transform(mke, crs = st_crs(dist)),
              color = cfc_orange, fillOpacity = .1,
              weight = 2) |> 
  fitBounds(-88.1, 42.925, -87.9, 43.15)
...
enr_by_sector <- cc |> 
  ...
  ggplot(aes(organisation_type, enr)) +
