...
if (params$honorific == "Representative") {
  leges_sf |> 
    ...
    geom_sf_text(aes(label = district), color = "grey70", size = 2) +
    geom_sf(data = leges_sf |> filter(district == params$district)) +
...
if (params$honorific == "Representative") {
  t <- leges_sf |> 
    filter(district == params$district & house == "Assembly") |> 
  ...
  t <- leges_sf |> 
    filter(district == params$district & house == "Senate") |> 
    st_intersection(geo_schools) |> 
    as_tibble() |> 
    select(-c(overall_rating, overall_score)) |> 
    left_join(make_mke_rc() |> 
